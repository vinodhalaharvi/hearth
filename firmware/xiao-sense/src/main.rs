...
fn main() -> Result<()> {
    ...
    // Onboard user LED. On the Seeed XIAO ESP32-S3 this is GPIO21 and is
    // active-low: driving the pin LOW turns the LED on, HIGH turns it off.
    ...
    loop {
        // liveness blink (active-low: LOW = on, HIGH = off)
        led.set_low()?;
